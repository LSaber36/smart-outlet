...
void getADCReading()
{
 ...
 while (millis() - time_check < 1000)
  {
    ADCResult = ads.readADC_Differential_0_1() * ADCMultiplier/1000;
    if(maxv<ADCResult)maxv=ADCResult;
    if(minv>ADCResult)minv=ADCResult;
  }
  if ((averageVoltage-minv)>(maxv-averageVoltage))maxv=averageVoltage+averageVoltage-minv;
  ADCValue = (maxv-averageVoltage)*10;
  
  //Ignore small voltage differences
  if((maxv-minv)<.1)ADCValue=0;  
